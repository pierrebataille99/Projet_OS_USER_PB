        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
				...
				if (nbClients==4)
				{
					...
					for (int k = 0; k < 8; k++) {
						sprintf(reply, "V %d %d %d", 0, k, tableCartes[0][k]);
						sendMessageToClient(tcpClients[0].ipAddress, tcpClients[0].port, reply);
					}
					...
					for (int k = 0; k < 8; k++) {
						sprintf(reply, "V %d %d %d", 1, k, tableCartes[1][k]);
						sendMessageToClient(tcpClients[1].ipAddress, tcpClients[1].port, reply);
					}
					...
					for (int k = 0; k < 8; k++) {
						sprintf(reply, "V %d %d %d", 2, k, tableCartes[2][k]);
						sendMessageToClient(tcpClients[2].ipAddress, tcpClients[2].port, reply);
					}
					...
					for (int k = 0; k < 8; k++) {
						sprintf(reply, "V %d %d %d", 3, k, tableCartes[3][k]);
						sendMessageToClient(tcpClients[3].ipAddress, tcpClients[3].port, reply);
					}
...
/// Structure de gestion d’une erreur fatale : affiche un message d’erreur système, puis termine le programme.
void error(const char *msg)
{
    perror(msg);
    exit(1);
...
/// Mélange aléatoirement le deck en permutant deux cartes 1000 fois.
void melangerDeck()
{
        int i;
        int index1,index2,tmp;
...
/// Initialise la table de possessions tableCartes.
/// tableCartes[i][j] = nombre d’objets de symbole j possédés par le joueur i.
/// Le deck est réparti trois cartes par joueur, la 13ᵉ carte (indice 12) est le coupable.
void createTable()
{
	// Le joueur 0 possede les cartes d'indice 0,1,2
	// Le joueur 1 possede les cartes d'indice 3,4,5 
...
/// Affiche l’ordre actuel du deck et la table de possessions.
void printDeck()
{
        int i,j;
...
/// Affiche la liste des clients connectés (ip, port, nom).
void printClients()
{
        int i;
...
/// Cherche un client par nom et retourne son index, ou -1 si non trouvé.
int findClientByName(char *name)
{
        int i;
...
/// Envoie un message texte à un client donné (ip, port).
void sendMessageToClient(char *clientip,int clientport,char *mess)
{
    int sockfd, portno, n;
    struct sockaddr_in serv_addr;
...
int main(int argc, char *argv[])
{
	...
	/// Affichage initial du deck avant mélange
	printDeck();
	/// Mélange du deck et création de la table des cartes
	melangerDeck();
	createTable();
	/// Affichage du deck et de la table après préparation
	printDeck();
	joueurCourant=0;
	//fsmServer = 0;

	/// Initialisation des structures clients en attente de connexion
	for (i=0;i<4;i++)
	{
        	strcpy(tcpClients[i].ipAddress,"localhost");
        	tcpClients[i].port=-1;
     ...
     {    
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
				case 'C':  // demande de connexion d’un nouveau client
					sscanf(buffer,"%c %s %d %s", &com, clientIpAddress, &clientPort, clientName);
					printf("COM=%c ipAddress=%s port=%d name=%s\n",com, clientIpAddress, clientPort, clientName);
					...
					if (nbClients==4)
					{
						/// Distribution des cartes et informations de tableCartes pour chaque joueur
						...
						// Définir le joueur courant (premier tour)
						sprintf(reply, "M %d", joueurCourant);
						broadcastMessage(reply);

						fsmServer=1;  // passage au mode jeu lancé
					}
					break;
                }
	}
	else if (fsmServer==1)     // partie lancée, gestion des commandes de jeu
	{
		switch (buffer[0])
		{
			// Cas G : accusation directe de la carte coupable
			case 'G':
			{
				int senderId, guess;
				// On lit l'expéditeur et la carte accusée
				sscanf(buffer, "G %d %d", &senderId, &guess);
				...
				if (guess == deck[12]) {
					...
					// mauvais suspect : on passe au joueur suivant
					joueurCourant = (joueurCourant + 1) % 4;
					sprintf(reply, "M %d", joueurCourant);
					broadcastMessage(reply);
				}
			...
			// Cas O : question globale sur un symbole, broadcast du résultat
			case 'O':
			{
				int senderId, symbole;
				sscanf(buffer, "O %d %d", &senderId, &symbole);
			...
			// Cas S : question ciblée sur un symbole à un joueur précis
			case 'S':
			{
				int senderId, cible, symbole;
				sscanf(buffer, "S %d %d %d", &senderId, &cible, &symbole);

				// On envoie uniquement au questionneur la quantité détenue
				int quantite = tableCartes[cible][symbole];
				sprintf(reply, "V %d %d %d", cible, symbole, quantite);
				sendMessageToClient(
					tcpClients[senderId].ipAddress,
